      ...
      <div class="menu-wrapper" ref="menuWrapper">
        <ul>
          ...
          <span class="text border-1px">
            <span v-if="item.type < 5" class="icon" :class="classMap[item.type]"></span>
            {{item.name}}
          </span>
          </li>
        </ul>
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    created() {
      //微信登录列成处理分析：
     /* 1、微信访问http://4uv4xq.natappfree.cc
      ...
      4、访问上述地址后获取到openid并跳转到http://4uv4xq.natappfree.cc/#/?openid=获取到的openid
      5、页面会再次出啊新，此时openid已经有值了，将openid缓存到cookie并设置过期时间
     ...
     */
      ...
      //alert("isTrue:"+(getCookie('openid') == null));*/
      ...
      this.$http.get('http://t238997p11.qicp.vip/sell/buyer/product/list').then((response) => {
